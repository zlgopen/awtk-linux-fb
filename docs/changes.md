...
2026/09/17
  * 修改编译参数OPENGL_ANTIALIAS默认使用SW模式，在嵌入式Linux设备运行时性能更好(感谢智明提供补丁)

2026/09/15
  * 支持 text shaping(感谢泽武提供补丁)

2026/07/10
  * 修改tslib默认为非阻塞模式打开，调整ts_read执行顺序，避免部分设备（使用老版本tslib）出现issues/123问题(感谢俊杰测试)
  * 优化ts_read返回值处理和热插拔逻辑，避免不必要的sleep卡顿

2026/05/11
  * 配合最新版本awtk，完善编译脚本，增加对SheenBidi库编译支持(感谢智明提供补丁)

2026/01/08
  * 增加exit_notifier机制，使得Ctrl-C时可以通知所有输入设备线程走正常退出流程，避免内存泄漏

2025/12/29
  * 配合最新版本awtk，在程序退出时调用native_window_fb_gl_deinit避免内存泄漏(感谢智明提供补丁)
