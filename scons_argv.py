...
COMPILE_CONFIG = {
  ...
  'OS_LIBPATH' : { 'value' : [], 'desc' : ['compile lib paths', '..example: OS_LIBPATH = ["/path/to/libdir1", "/path/to/libdir2"]'], 'help_info' : 'set compile\'s lib paths, so care of system and compile tools, use \',\' split muliple librarie\'s paths '},
  'OS_CPPPATH' : { 'value' : [], 'desc' : ['compile include paths', '..example: OS_CPPPATH = ["/path/to/incdir1", "/path/to/incdir2"]'], 'help_info' : 'set compile\'s include paths, so care of system and compile tools, use \',\' split muliple include path '},
  ...
  'TOOLS_PREFIX' : { 'value' : None, 'desc' : ['compile tools prefix path', '..example: TOOLS_PREFIX = "/path/to/arm-linux-gnueabihf-"'], 'help_info' : 'set compile tools prefix path, value allow set \'\''},
  'TSLIB_LIB_DIR' : { 'value' : None, 'desc' : ['tslib lib path', '..example: TSLIB_LIB_DIR = "/path/to/tslib/lib"'], 'help_info' : 'set use tslib lib path, value allow set \'\''},
  'TSLIB_INC_DIR' : { 'value' : None, 'desc' : ['tslib include path', '..example: TSLIB_INC_DIR = "/path/to/tslib/include"'], 'help_info' : 'set use tslib lib path, value allow set \'\''},
  'ENABLE_CURSOR' : { 'value' : True, 'desc' : ['enable cursor mouse'], 'help_info' : 'set enable cursor mouse, value is true or false'},
  'INPUT_ENGINE' : { 'value' : None, 'desc' : ['null/spinyin/t9/t9ext/pinyin', '..example: INPUT_ENGINE = "pinyin"'], 'help_info' : 'set awtk use input engine' },
  'VGCANVAS' : { 'value' : None, 'desc' : ['NANOVG/NANOVG_PLUS/CAIRO', '..example: VGCANVAS = "NANOVG"'], 'help_info' : 'set awtk use render vgcanvas type' },
  'DEBUG' : { 'value' : False, 'desc' : ['awtk\'s compile is debug'], 'help_info' : 'awtk\'s compile is debug, value is true or false' },
  'APP' : { 'value' : None, 'save_file' : False, 'desc' : ['build this app'], 'help_info' : 'set app is build, value is app\'s root' },
  'LCD_DEVICES' : { 'value' : None, 'desc' : ['linux\'s lcd devices type, value is fb/drm/egl_for_fsl/egl_for_x11/egl_for_gbm', '..example: LCD_DEVICES = "fb"'], 'help_info' : 'when building, use linux\'s lcd devices type, value is fb/drm/egl_for_fsl/egl_for_x11/egl_for_gbm' },
