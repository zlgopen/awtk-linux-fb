/**
 * File:   wayland_input_devices.c
 * Author: AWTK Develop Team
 * Brief:  wayland input devices
 *
...
static void keymap_format_cb(void* data, struct wl_keyboard* keyboard, uint32_t format,
                             int32_t fd, uint32_t keymap_size) {
  wayland_keyboard_t* kbd = data;
...
static void key_cb(void* data, struct wl_keyboard* keyboard, uint32_t serial,
                   uint32_t time, uint32_t key, uint32_t state) {
  (void) keyboard;
  (void) serial;
  (void) time;
  wayland_keyboard_t* keydata = data;
  if (keydata->keyboard_dispatch) {
    keydata->keyboard_dispatch(state, key);
  }
}
...
static void motion_pointer_cb(void* data, struct wl_pointer* pointer, uint32_t time,
  ...
  input_bundle_t* input = data;
  if (input->mouse.pointer_dispatch) {
    input->mouse.pointer_dispatch(POINTER_BUTTON_STATE_MOTION, 0, 
...
static void touch_down_cb(void* data, struct wl_touch* wl_touch, uint32_t serial, uint32_t time,
  ...
  wayland_touch_t* point = data;
  if (point->pointer_dispatch) {
    point->pointer_dispatch(WL_POINTER_BUTTON_STATE_PRESSED, BTN_LEFT,
...
static void touch_up_cb(void* data, struct wl_touch* wl_touch,
  ...
  wayland_touch_t* point = data;
  if (point->pointer_dispatch) {
    point->pointer_dispatch(WL_POINTER_BUTTON_STATE_RELEASED, BTN_LEFT, -1, -1);
  }
}
...
static void touch_motion_cb(void* data, struct wl_touch* wl_touch, uint32_t time,
  ...
  wayland_touch_t* point = data;
  if (point->pointer_dispatch) {
    point->pointer_dispatch(POINTER_BUTTON_STATE_MOTION, 0,
                            wl_fixed_to_int(x), wl_fixed_to_int(y));
  }
}
