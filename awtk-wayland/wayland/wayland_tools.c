...
static void global_registry_handler(void* data, struct wl_registry* registry,
  ...
  if (strcmp(interface, wl_compositor_interface.name) == 0) {
    ...
    zwp_fullscreen_shell_v1_add_listener(objs->fullscreen_shell, &fullscreen_shell_listener, objs);
...
WAYLAND_SETUP_ERR setup_wayland(wayland_data_t* objs, bool_t fullscreen) {
  ...
  if (objs->xdg_wm_base != NULL) {
    ...
    xdg_surface_add_listener(objs->xdg_surface, &xdg_surface_listener, objs);
...
buffer_t* wayland_create_double_buffer(struct wl_shm *shm, int width, int height) {
  ...
  buffer->bufs = malloc(sizeof(double_buffer_list_t));
